...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!type || (type !== 'solar' && type !== 'wind')) {
      ...
      } as NationalMapAPIResponse, { status: 400 });
    }
    ...
    // Note: In production, this should be optimized with caching and batch requests
    ...
    const validPoints = results.filter((p): p is GridPoint => p !== null);
    ...
    if (validPoints.length === 0) {
      ...
      } as NationalMapAPIResponse, { status: 404 });
    }
    ...
    return NextResponse.json({
      ...
      error: {
        ...
        message: error.message || 'An error occurred while generating the national map',
        details: process.env.NODE_ENV === 'development' ? error.stack : undefined
      }
    } as NationalMapAPIResponse, { status: 500 });
  }
}
