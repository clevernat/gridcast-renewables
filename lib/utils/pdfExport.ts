...
export async function generateAtmosphericResearchPDF(
  ...
  doc.text("Atmospheric Science Research Report", 105, yPos, { align: "center" });
  
  yPos += 15;
  doc.setFontSize(12);
  doc.setFont("helvetica", "normal");
  doc.text("GridCast Renewables - Atmospheric Analysis Platform", 105, yPos, { align: "center" });
  ...
  yPos += 6;
  doc.text(`Coordinates: ${forecast.location.latitude.toFixed(4)}°N, ${forecast.location.longitude.toFixed(4)}°W`, 20, yPos);
  yPos += 6;
  ...
  if (researchData) {
    ...
    const qualityData = [
      ...
      ["Missing Data", `${researchData.dataQuality.missingDataPercentage.toFixed(1)}%`],
  ...
  if (researchData && yPos < 250) {
    ...
    const statsData: any[] = [["Variable", "Mean", "Std Dev", "Min", "Max", "Data Points"]];
    
    const keyVars = ["temperature", "surfacePressure", "relativeHumidity", "windSpeed", "solarIrradiance"];
  ...
  if (researchData?.correlations) {
    ...
    const corrData: any[] = [["Variable", ...researchData.correlations.variables]];
    ...
    doc.text("Note: Values range from -1 (perfect negative correlation) to +1 (perfect positive correlation)", 20, yPos);
  }
  ...
  if (researchData?.trends && researchData.trends.length > 0) {
    ...
    const trendData: any[] = [["Variable", "Trend", "Slope", "R²", "P-Value", "Significance"]];
  ...
  if (researchData?.anomalies && researchData.anomalies.length > 0) {
    ...
    doc.setFontSize(14);
    doc.setFont("helvetica", "bold");
    doc.text(`Anomalies Detected (${researchData.anomalies.length} total)`, 20, yPos);
    yPos += 8;
    
    const anomalyData: any[] = [["Timestamp", "Variable", "Value", "Expected", "Deviation (σ)", "Severity"]];
  ...
  if (forecast) {
    ...
    const avgCapacity = forecast.outputs.reduce((sum, o) => sum + (o.capacity || 0), 0) / forecast.outputs.length;
    
    doc.setFontSize(10);
    doc.setFont("helvetica", "normal");
    doc.text(`Total Energy Production: ${totalEnergy.toFixed(2)} ${forecast.asset.type === "solar" ? "kWh" : "MWh"}`, 20, yPos);
    yPos += 6;
    doc.text(`Average Capacity Factor: ${avgCapacity.toFixed(1)}%`, 20, yPos);
    yPos += 6;
    doc.text(`Peak Power: ${Math.max(...forecast.outputs.map(o => o.power)).toFixed(2)} ${forecast.asset.type === "solar" ? "kW" : "MW"}`, 20, yPos);
    yPos += 12;
    
    // Hourly forecast table (first 24 hours)
    const forecastData: any[] = [["Time", "Power", "Capacity %", "Temperature", "Wind Speed", "Solar Irr."]];
  ...
  if (longTerm) {
    ...
    doc.setFontSize(10);
    doc.setFont("helvetica", "normal");
    doc.text(`Annual Production: ${longTerm.annualProduction.toFixed(2)} ${longTerm.asset.type === "solar" ? "kWh/year" : "MWh/year"}`, 20, yPos);
    yPos += 6;
    doc.text(`Average Capacity Factor: ${longTerm.averageCapacityFactor.toFixed(1)}%`, 20, yPos);
    yPos += 12;
    
    const monthlyData: any[] = [["Month", "Avg Production", "Capacity Factor %"]];
  ...
  // Save the PDF
  const filename = `atmospheric_research_${forecast.location.latitude.toFixed(2)}_${forecast.location.longitude.toFixed(2)}_${Date.now()}.pdf`;
  doc.save(filename);
}
 ...
 */
export async function generateQuickSummaryPDF(forecast: PowerForecast): Promise<void> {
  const doc = new jsPDF();
  ...
  doc.setFontSize(10);
  doc.setFont("helvetica", "normal");
  doc.text(`Location: ${forecast.location.address || `${forecast.location.latitude.toFixed(4)}°N, ${forecast.location.longitude.toFixed(4)}°W`}`, 20, 35);
  doc.text(`Generated: ${new Date().toLocaleString()}`, 20, 42);
  
  const totalEnergy = forecast.outputs.reduce((sum, o) => sum + o.power, 0);
  doc.text(`Total Energy (48h): ${totalEnergy.toFixed(2)} ${forecast.asset.type === "solar" ? "kWh" : "MWh"}`, 20, 55);
